...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < stringCCN.length; i += 1) {
    let cardNumber = parseInt(stringCCN[i], 10);
...
function isBracketsBalanced(str) {
  ...
  const result = [];
  for (let i = 0; i < str.length; i += 1) {
    const index = bracketsArray.indexOf(str[i]);
    if (index % 2 === 0) {
      result.push(str[i]);
    } else if (result[result.length - 1] === bracketsArray[index - 1]) {
      result.pop();
    } else {
      return false;
    }
...
function getCommonDirectoryPath(pathes) {
  ...
  const doppelt = joinedPathArr
    .split('/')
    .filter((elem, pos, arr) => pos !== arr.indexOf(elem))
    .join('/');

  const result = `/${doppelt}/`;

  const correct = (element) => element.substr(0, 1) !== '/';

  if (!pathes.some(correct)) {
    if (result.length - 1 <= 1) return '/';
    return result;
  }
  return '';
}
